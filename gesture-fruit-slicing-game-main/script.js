...
const BOSS_TYPE = { emoji: '👑', color: 0xffd700, name: 'boss' };
...
// ── Levels ──────────────────────────────────────────────────────────────────
const LEVEL_THRESHOLDS = [0, 500, 1200, 2500, 5000, 8000, 12000, 20000];
const LEVEL_COLORS = [
  '#00f5ff', // L1: Cyan
  '#39ff14', // L2: Green
  '#ffe600', // L3: Yellow
...
const GOLDEN_CHANCE      = 0.03;  // 3% chance for a golden fruit
const BOSS_LEVEL_INTERVAL = 5;    // Boss appears every 5 levels
...
window.FingerCursor = (() => {
  ...
  function _onResults(results) {
    ...
    // Detect gestures
    GestureHandler.detect(results.multiHandLandmarks[0], scene);
...
function spawnFruit() {
  ...
  // ── Golden fruit ──────────────────────────────────────────────────────────
  if (roll < BOMB_CHANCE + POWERUP_CHANCE + GOLDEN_CHANCE) {
    const W = window.innerWidth, H = window.innerHeight;
    const x = Phaser.Math.Between(W * 0.1, W * 0.9);
    const textureKey = scene.fruitTextures['strawberry']; // placeholder color
    const sprite = scene.physics.add.sprite(x, H + 40, textureKey);
    sprite.setCircle(FRUIT_SIZE/2).setAlpha(0).setDepth(1);
    const label = scene.add.text(x, H + 40, '⭐', { fontSize: `${FRUIT_SIZE}px` }).setOrigin(0.5, 0.5).setDepth(2);
    const glow = scene.add.circle(x, H + 40, FRUIT_SIZE * 0.8, 0xffd700, 0).setStrokeStyle(4, 0xffe600, 0.8).setDepth(1);
    scene.tweens.add({ targets: glow, angle: 360, duration: 1000, repeat: -1 });
    sprite.body.setVelocity(Phaser.Math.Between(-150, 150), Phaser.Math.Between(-800, -600));
    fruits.push({ sprite, text: label, glow, kind: 'golden', type: { color: 0xffd700, name: 'golden', emoji: '⭐' }, sliced: false, spinSpeed: 300 });
    return;
  }
...
function checkSlice(currX, currY) {
  ...
  for (let i = fruits.length - 1; i >= 0; i--) {
    ...
    if (fruit.kind === 'boss') {
      BossSystem.hit(fruit, scene, currX, currY);
      continue;
    }
...
function sliceFruit(index) {
  ...
  if (fruit.kind === 'golden') {
    addScore(150, cx, cy);
    AchievementSystem.trigger('Gold Digger');
    // Screen flash
    const flash = scene.add.rectangle(window.innerWidth/2, window.innerHeight/2, window.innerWidth, window.innerHeight, 0xffe600, 0.3);
    flash.setDepth(100);
...
function checkLevelUp() {
  ...
  if (nextThreshold !== undefined && score >= nextThreshold) {
    ...
    if (currentLevel % BOSS_LEVEL_INTERVAL === 0) {
      setTimeout(() => BossSystem.spawn(window._phaserScene), 2000);
    }
  }
}
...
function levelUp() {
  ...
  if (scene) {
    const W = window.innerWidth, H = window.innerHeight;
    const txt = scene.add.text(W/2, H/2, `LEVEL ${currentLevel}`, {
      fontFamily: 'Boogaloo, cursive',
      fontSize: '120px', color: '#fff', stroke: '#000', strokeThickness: 10
    });
    txt.setOrigin(0.5, 0.5).setDepth(100).setScale(0);
    
    scene.tweens.add({
      targets: txt, scaleX: 1, scaleY: 1, duration: 600, ease: 'Back.easeOut',
      onComplete: () => {
        scene.tweens.add({
          targets: txt, alpha: 0, y: H/2 - 100, delay: 1000, duration: 500,
          onComplete: () => txt.destroy()
        });
      }
    });

    // Flash screen with level color
    const levelColor = LEVEL_COLORS[(currentLevel - 1) % LEVEL_COLORS.length];
    const flash = scene.add.rectangle(W/2, H/2, W, H, 
      Phaser.Display.Color.HexStringToColor(levelColor).color, 0.15);
    flash.setDepth(50);
    scene.tweens.add({
      targets: flash, alpha: 0, duration: 800, onComplete: () => flash.destroy()
    });
  }
}
...
function startGame() {
  ...
  score           = LEVEL_THRESHOLDS[currentLevel - 1] || 0;
  lives           = MAX_LIVES;
  combo           = 0;
  rapidSliceCount = 0;
  spawnDelay      = Math.max(MIN_INTERVAL, SPAWN_INTERVAL - (currentLevel - 1) * 100);
  gameRunning     = true;
...
const BossSystem = (() => {
  function spawn(scene) {
    ...
    if (!scene.textures.exists('boss_tex')) {
       const gfx = scene.add.graphics();
       gfx.fillStyle(0xffd700, 1);
       gfx.fillCircle(size/2, size/2, size/2);
       gfx.generateTexture('boss_tex', size, size);
       gfx.destroy();
    }

    const sprite = scene.physics.add.sprite(x, H + 100, 'boss_tex');
    sprite.setCircle(size / 2);
    sprite.setAlpha(0);
    sprite.setDepth(10);
    sprite.body.setAllowGravity(false);
    
    const label = scene.add.text(x, H + 100, BOSS_TYPE.emoji, {
      fontSize: `${size}px`, align: 'center'
    });
    label.setOrigin(0.5, 0.5).setDepth(11);

    const healthBar = scene.add.rectangle(x, H + 200, 150, 10, 0xff0000);
    healthBar.setDepth(12);

    scene.tweens.add({
      targets: [sprite, label, healthBar],
      y: H / 2,
      duration: 1500,
      ease: 'Back.easeOut',
    ...
    const bossObj = {
      sprite, text: label, bar: healthBar,
      kind: 'boss',
      health: 10 + (currentLevel * 2),
      maxHealth: 10 + (currentLevel * 2),
      type: BOSS_TYPE,
      sliced: false,
      spinSpeed: 50
    };
  ...
  function hit(fruit, scene, x, y) {
    fruit.health--;
    ScreenShake.trigger(scene, 10, 100);
    const pct = Math.max(0, fruit.health / fruit.maxHealth);
    fruit.bar.width = 150 * pct;
    
    const hitText = scene.add.text(x, y, 'HIT!', {
      fontFamily: 'Boogaloo, cursive',
      fontSize: '40px', color: '#fff', stroke: '#000', strokeThickness: 4
    });
    scene.tweens.add({
      targets: hitText, y: y - 80, alpha: 0, duration: 400, onComplete: () => hitText.destroy()
    });

    if (fruit.health <= 0) {
      fruit.sliced = true;
      fruit.bar.destroy();
      addScore(1000 * currentLevel, fruit.sprite.x, fruit.sprite.y);
      AchievementSystem.trigger('Boss Slayer');
      setTimeout(() => {
        const idx = fruits.indexOf(fruit);
        if (idx !== -1) sliceFruit.call(scene, idx);
      }, 50);
    }
  }
...
const AchievementSystem = (() => {
  ...
  function trigger(name) {
    ...
    const container = document.getElementById('achievement-container') || createContainer();
...
const GestureHandler = (() => {
  ...
  function detect(landmarks, scene) {
    if (!gameRunning) return;
    const now = performance.now();
  ...
  function triggerPeace(scene) {
    SlowMotion.trigger(scene);
    AchievementSystem.trigger('Zen Master');
    const W = window.innerWidth, H = window.innerHeight;
    const txt = scene.add.text(W/2, H/2 + 100, '✌️ ZEN MODE', {
      fontFamily: 'Boogaloo, cursive',
      fontSize: '60px', color: '#00f5ff', stroke: '#000', strokeThickness: 6
    });
    txt.setOrigin(0.5, 0.5).setDepth(100);
    scene.tweens.add({
      targets: txt, y: H/2 + 50, alpha: 0, duration: 2000, onComplete: () => txt.destroy()
    });
  }
